...
cleanup() {
    ...
    trap - EXIT
    if [[ $status -ne 0 ]]; then
        for path in "$TMP/daemon.out" "$TMP/pm.log" "$TMP/prog.yaml"; do
            [[ -e $path ]] || continue
            echo "--- $path ---" >&2
...
printf '%s\n' \
    ...
    "pm_tiny_allowed_uids: [$NOBODY_UID]" \
...
# shellcheck disable=SC2024
sudo -n -u nobody env HOME=/tmp PATH=/usr/bin:/bin \
...
[[ $(sed -n '1p' "$TMP/peer_identity") == "$NOBODY_UID" ]]
[[ $(sed -n '2p' "$TMP/peer_identity") == "$(id -g nobody)" ]]
...
sudo -n -u nobody env HOME=/fake-home USER=fake-user LOGNAME=fake-logname SHELL=/fake-shell \
...
sudo -n -u nobody env HOME=/fake-home PATH=/usr/bin:/bin \
    PM_TINY_HOME="$TMP/home" PM_TINY_SOCK_FILE="$TMP/pm.sock" \
...
sudo -n grep -q "start request peer_pid=.*peer_uid=$NOBODY_UID.*name=root_environment.*target_user=root" "$TMP/pm.log"
