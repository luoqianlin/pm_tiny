...
std::string write_config_file(const std::string &dir,
                              ...
                              int log_max_size_kb = 8192,
                              int log_archive_count = 5) {
    const std::string cfg_path = dir + "/pm_tiny.yaml";
    std::ofstream ofs(cfg_path);
    ofs << "pm_tiny_home_dir: " << home << "\n";
    ofs << "pm_tiny_log_file: " << log << "\n";
    ofs << "pm_tiny_log_level: " << log_level << "\n";
    ofs << "pm_tiny_log_max_size_kb: " << log_max_size_kb << "\n";
    ofs << "pm_tiny_log_archive_count: " << log_archive_count << "\n";
    ...
    ofs << "pm_tiny_allowed_uids: [1234]\n";
    ofs << "pm_tiny_allowed_gids: [2345]\n";
    ofs.close();
...
void test_env_overrides_config() {
    const std::vector<std::string> env_names = {
            ...
            PM_TINY_LOG_LEVEL, "PM_TINY_LOG_MAX_SIZE_KB", "PM_TINY_LOG_ARCHIVE_COUNT"
    };
    ...
    env_guard.set(PM_TINY_LOG_LEVEL, "error");
    env_guard.set("PM_TINY_LOG_MAX_SIZE_KB", "16384");
    env_guard.set("PM_TINY_LOG_ARCHIVE_COUNT", "7");
    ...
    expect(cfg->log_level == "error", "env log level override failed");
    expect(cfg->log_max_size_kb == 16384, "env log max size override failed");
    expect(cfg->log_archive_count == 7, "env log archive count override failed");
...
void test_config_file_fallback() {
    ...
    expect(cfg->log_level == "warn", "cfg log level mismatch");
    expect(cfg->log_max_size_kb == 8192, "cfg log max size mismatch");
    expect(cfg->log_archive_count == 5, "cfg log archive count mismatch");
    expect(cfg->allowed_uids == std::vector<unsigned int>{1234}, "cfg allowed uid mismatch");
    expect(cfg->allowed_gids == std::vector<unsigned int>{2345}, "cfg allowed gid mismatch");
...
void test_invalid_log_config_rejected() {
    const std::string base_dir = make_temp_dir("/tmp/pm_tiny_helper_invalid_");
    const std::string home = base_dir + "/home";
    make_dir(home);
    const std::string cfg_path = write_config_file(base_dir, home, home + "/pm.log",
                                                   home + "/prog.yaml", home + "/logs",
                                                   home + "/environ", home + "/pm.sock", false,
                                                   "auto", "", "verbose");
    bool rejected = false;
    try {
        (void) pm_tiny::get_file_config(cfg_path);
    } catch (const std::runtime_error &) {
        rejected = true;
    }
    expect(rejected, "invalid daemon log level was accepted");
}

void test_relative_paths_use_config_directory() {
    const std::string base_dir = make_temp_dir("/tmp/pm_tiny_helper_relative_");
    const std::string cfg_path = write_config_file(base_dir, "home", "logs/pm_tiny.log",
                                                   "prog.yaml", "logs", "environ",
                                                   "pm_tiny.sock", false);
    auto cfg = pm_tiny::get_file_config(cfg_path);
    expect(cfg->pm_tiny_home_dir == base_dir + "/home", "relative home path mismatch");
    expect(cfg->pm_tiny_log_file == base_dir + "/logs/pm_tiny.log", "relative log path mismatch");
    expect(cfg->pm_tiny_prog_cfg_file == base_dir + "/prog.yaml", "relative program path mismatch");
    expect(cfg->pm_tiny_app_log_dir == base_dir + "/logs", "relative app log path mismatch");
    expect(cfg->pm_tiny_app_environ_dir == base_dir + "/environ", "relative environ path mismatch");
    expect(cfg->pm_tiny_sock_file == base_dir + "/pm_tiny.sock", "relative socket path mismatch");
}
...
int main() {
    ...
    test_invalid_log_config_rejected();
    test_relative_paths_use_config_directory();
    return 0;
}
