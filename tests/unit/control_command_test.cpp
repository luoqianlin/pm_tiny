...
int main() {
    ...
    pm_tiny::protocol_message start;
    start.type = static_cast<std::uint16_t>(pm_tiny::control_command::start);
    expect(pm_tiny::decode_control_request(start).success,
           "start payload must be decoded by the dedicated start decoder");
    ...
    pm_tiny::protocol_message info;
    info.type = static_cast<std::uint16_t>(pm_tiny::control_command::info);
    expect(pm_tiny::decode_control_request(info).success, "empty info request should decode");
    info.payload.push_back(1);
    expect(!pm_tiny::decode_control_request(info).success, "info payload should be rejected");
