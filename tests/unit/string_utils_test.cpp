...
void test_split_extension() {
    std::string name;
    std::string extension;
    std::tie(name, extension) = mgr::utils::splitext("a.txt");
    assert(name == "a" && extension == ".txt");
    std::tie(name, extension) = mgr::utils::splitext("a.");
    assert(name == "a" && extension == ".");
    std::tie(name, extension) = mgr::utils::splitext("a");
    assert(name == "a" && extension.empty());
    std::tie(name, extension) = mgr::utils::splitext(".txt");
    assert(name == ".txt" && extension.empty());
    std::tie(name, extension) = mgr::utils::splitext("a..txt");
    assert(name == "a." && extension == ".txt");
    std::tie(name, extension) = mgr::utils::splitext("..txt");
    assert(name == "..txt" && extension.empty());
