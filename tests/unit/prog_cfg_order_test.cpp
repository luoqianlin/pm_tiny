...
int main() {
    ...
    auto unsafe = std::vector<pm_tiny::prog_cfg_t>{config("../escape")};
    expect(!pm_tiny::validate_and_order_prog_cfgs(unsafe, error) &&
           error.find("Invalid program name") != std::string::npos,
           "unsafe program names should be rejected");
    auto too_long = std::vector<pm_tiny::prog_cfg_t>{config(std::string(129, 'a'))};
    expect(!pm_tiny::validate_and_order_prog_cfgs(too_long, error) &&
           error.find("at most 128") != std::string::npos,
           "overlong program names should be rejected");
    return 0;
}
