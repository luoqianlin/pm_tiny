...
- `log` 和 `start --log` 发送当前 generation 最近 64 KiB 后实时跟随；每会话队列限制为 1 MiB。
...
| IPC 调用方权限 | `SO_PEERCRED`，支持 UID/GID allowlist | SDDL ACL，默认允许 `SYSTEM` 和管理员 | 均在接受控制连接时校验，不开放默认远程控制面 |
...
| 发布与升级 | 已有 Ubuntu 安装路径 | 尚无标准 MSVC 发布包、原地升级和自动回滚 | Windows 发布闭环仍需补齐 |

后续缩小差异时，优先考虑 Windows 指定用户运行、ConPTY 可行性、
`failure_action` 跨平台语义以及 MSVC 发布与回滚流程。`oom_score_adj` 不应在 Windows 创建无实际作用的
...
服务用户 SID。服务实例支持 `-PipeName`，避免测试和多实例之间复用管道。安装脚本会把
...
```powershell
cmake -S . -B build_msvc -G "Visual Studio 17 2022" -A x64 -DPM_TINY_BUILD_TESTS=ON
cmake --build build_msvc --config Release --parallel 4
ctest --test-dir build_msvc -C Release --output-on-failure
```

完整 CTest 必须包含 `windows_protocol_integration` 和 MSVC 专属的
`windows_service_integration`。生命周期公共代码变更还应运行 `termination_job_test` 和
`control_operation_test`。构建后检查：

```powershell
Test-Path .\build_msvc\Release\pm_tiny.exe
Test-Path .\build_msvc\Release\pm.exe
Test-Path .\build_msvc\Release\pm2.exe
```
...
- 测试失败后先检查 `build_msvc/test-artifacts/windows` 和 `windows-service`，再检查是否残留同名 pipe、临时服务或测试进程。
