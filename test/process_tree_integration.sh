...
"$BIN/pm" start "$BIN/test/process_tree_fixture fork $TMP/fork.pids" \
    --name tree_fork --no_daemon --no_pty --kill_timeout 1 >/dev/null
sleep .2
"$BIN/pm" stop tree_fork >/dev/null
sleep .2
assert_gone "$TMP/fork.pids"

"$BIN/pm" start "$BIN/test/process_tree_fixture orphan $TMP/orphan.pids" \
    --name tree_orphan --no_daemon --no_pty --kill_timeout 1 >/dev/null
