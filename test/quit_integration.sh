...
"$BIN/pm" start '/usr/bin/sleep 300' --name long_running >/dev/null
...
wait_socket "$PM_TINY_SOCK_FILE"
"$BIN/pm" start '/usr/bin/sleep 300' --name zombie_child >/dev/null
...
response = b'PMT2' + bytes((2, 1)) + header[6:8] + header[8:12] + struct.pack('>I', len(payload)) + payload
